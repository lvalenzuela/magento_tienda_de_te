	...
	try {
		jQuery.ajax( {
			...
			success : function(data) { 
				...
				if (data.status != 'ERROR' && jQuery('.sidebar-block-wishlist').length){
					jQuery('.block-wishlist').replaceWith(data.sidebar);
				} 
				if (data.status != 'ERROR' && jQuery('.block-wishlist-header').length == 0){
					jQuery('.top-wishlist').append(data.header_block);
					jQuery('.wishlist_empty').hide();
				} 
				if (data.status != 'ERROR' && jQuery('.block-wishlist-header').length){ 
					jQuery('.block-wishlist-header').replaceWith(data.header_block);
				} 
			}
		});
	} catch (e) {
