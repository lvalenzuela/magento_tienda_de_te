 ...
 */
class MagenThemes_MTBakery_Model_Observer
{
	/**
	...
	public function configSave()
	{
		...
		if ($section == 'mtbakery_design')
		{
			$websiteCode = Mage::app()->getRequest()->getParam('website');
			$storeCode = Mage::app()->getRequest()->getParam('store');
			
			Mage::getSingleton('mtbakery/cssgen_generator')->generateCss('design', $websiteCode, $storeCode);
        }else if($section == 'mtbakery'){
            $websiteCode = Mage::app()->getRequest()->getParam('website');
            $storeCode = Mage::app()->getRequest()->getParam('store');

            Mage::getSingleton('mtbakery/cssgen_generator')->generateCss('layout', $websiteCode, $storeCode);
        }
	}
    ...
    public function catalogBlockProductCollectionBeforeToHtml(Varien_Event_Observer $observer)
    {
        ...
        if ($productCollection instanceof Varien_Data_Collection) {
            ...
            Mage::getModel('mtbakery/collection')->appendType($productCollection);
        }
