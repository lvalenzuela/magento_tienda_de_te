...
?>
<?php
class MagenThemes_MTBakery_Block_Mainmenu_Menu extends Mage_Catalog_Block_Navigation
{
    protected $_column;
    protected $_isMomenu;
    protected $_isSmart;
                                                   ...
                                                   $isOutermost = false, $outermostItemClass = '', $childrenWrapClass = '', $noEventAttributes = false)
    {
        ...
        if (Mage::helper('mtbakery')->getCfg('menu/show_if_no_children')) {$showblock = true; }
    ...
    protected function _getCategoryLabelHtml($category, $level){
        ...
        if ($labelCategory){
            $getLabel = trim(Mage::helper('mtbakery')->getCfg('menu/' . $labelCategory));
            if ($getLabel) {
                if ($level == 0){
