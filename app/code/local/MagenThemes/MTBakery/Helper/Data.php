...
?>
<?php
class MagenThemes_MTBakery_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    {
        return Mage::getStoreConfig('mtbakery/' . $optionString);
    }
    ...
    {
        if ($storeId)
            return Mage::getStoreConfig('mtbakery_design', $storeId);
        else
            return Mage::getStoreConfig('mtbakery_design');
    }
    ...
    {
        return Mage::getStoreConfig('mtbakery_design/' . $optionString, $storeCode);
    }
    ...
    {
        return Mage::getStoreConfig('mtbakery_layout/' . $optionString, $storeCode);
    }
     ...
     public function getLabel(Mage_Catalog_Model_Product $product)
    {
        ...
        if ( $this->getCfg("product_labels/new") && $this->_checkNew($product) ) {
            $html .= '<span class="product-'.$this->getCfg("product_labels/new").'-label">'.$this->__('New').'</span>';
        }
        if ( $this->getCfg("product_labels/sale") && $this->_checkSale($product) ) {
            $html .= '<span class="product-'.$this->getCfg("product_labels/sale").'-label">'.$this->__('Sale').'</span>';
        }
	 ...
	 public function getAltImgHtml($product, $w, $h, $imgVersion='small_image')
    {
        ...
        if ($gal = $product->getMediaGalleryImages())
        {
            ...
            {
                return
                    '<img class="img-responsive alt-img lazy lazyOwl " data-src="' . Mage::helper('mtbakery/image')->getImg($product, $w, $h, $imgVersion, $altImg->getFile()) . '" alt="' . $product->getName() . '" />';
            }
        }
    ...
    {
        $config['zoomWidth'] = is_numeric(Mage::getStoreConfig('mtbakery/product_page/zoom_width'))?Mage::getStoreConfig('mtbakery/product_page/zoom_width'):'auto';
        $config['zoomHeight'] = is_numeric(Mage::getStoreConfig('mtbakery/product_page/zoom_height'))?Mage::getStoreConfig('mtbakery/product_page/zoom_height'):'auto';
        $config['position'] = Mage::getStoreConfig('mtbakery/product_page/zoom_position')?Mage::getStoreConfig('mtbakery/product_page/zoom_position'):'right';
        $config['adjustX'] = is_numeric(Mage::getStoreConfig('mtbakery/product_page/zoom_adjustX'))?Mage::getStoreConfig('mtbakery/product_page/zoom_adjustX'):0;
        $config['adjustY'] = is_numeric(Mage::getStoreConfig('mtbakery/product_page/zoom_adjustY'))?Mage::getStoreConfig('mtbakery/product_page/zoom_adjustY'):0;
        if ($rel){
            $out = array();
