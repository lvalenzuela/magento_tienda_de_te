...
?>
<?php
class MagenThemes_MTBakery_Model_Cssgen_Generator extends Mage_Core_Model_Abstract{
    ...
    protected function _generateStoreCss($design, $storeCode){
        ...
        $filedefault = Mage::helper('mtbakery/cssgen')->getGeneratedCssDir() . $filename;
        $path = 'magenthemes/mtbakery/css/' . $design . '.phtml';
        ...
        try{
            ...
            if (empty($block)) {
                throw new Exception( Mage::helper('mtbakery')->__("Template file is empty or doesn't exist: %s", $path) );
            }
            $file = new Varien_Io_File();
            $file->setAllowCreateFolders(true);
            $file->open(array( 'path' => Mage::helper('mtbakery/cssgen')->getGeneratedCssDir() ));
            ...
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('mtbakery')->__('Failed generating CSS file: %s in %s', $filename, Mage::helper('mtbakery/cssgen')->getGeneratedCssDir()). '<br/>Message: ' . $gener->getMessage());
