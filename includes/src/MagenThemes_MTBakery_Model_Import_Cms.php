...
class MagenThemes_MTBakery_Model_Import_Cms extends Mage_Core_Model_Abstract
{
	private $_importPath;
    ...
    {
        parent::__construct();
		$this->_importPath = Mage::getBaseDir() . '/app/code/local/MagenThemes/MTBakery/etc/import/';
    }
        ...
        try
        {
            ...
            foreach ($xmlObj->getNode($itemContainerNodeString)->children() as $item)
            {
                ...
                foreach($item->page as $object){
                    Mage::getSingleton('mtbakery/resource_widget')->importInstancePage($model->getInstanceId(), $object, $item->sort_order, $item->xml);
                }
                $i++;
            }
            //Final info
